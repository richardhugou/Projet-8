...
if df.empty:
    ...
    with col_left:
        ...
        fig_lat = px.line(df, x='timestamp', y='latency', template="plotly_dark", color_discrete_sequence=['#00f2ff'])
        fig_lat.add_hline(y=50, line_dash="dash", line_color="red", annotation_text="SLA 50ms")
        st.plotly_chart(fig_lat, use_container_width=True)

    with col_right:
        st.subheader("Distribution des Scores")
        fig_hist = px.histogram(df, x='probability', color='status', barmode='overlay', 
                               template="plotly_dark", color_discrete_map={'ACCORDÉ': '#00cc96', 'REFUSÉ': '#ef553b'})
    ...
    with tab1:
        if os.path.exists(REPORT_HTML):
            ...
            st.info("Le rapport HTML n'a pas encore été généré par `drift_analysis.py`.")
...
# Footer
st.markdown("---")
st.caption(f"Dernière mise à jour : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Projet 8 - MLOps Industrialisation")
